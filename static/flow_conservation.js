(function() {
  ...
  function initFlowConservationDemo(boardsData) {
    ...
    defs.append('marker').attr('id','mBlack').attr('viewBox','0 0 10 10')
      ...
      .attr('orient','auto').append('path').attr('d','M0,0L0,10L10,5Z').attr('fill','#000');
    defs.append('marker').attr('id','mGreen').attr('viewBox','0 0 10 10')
      .attr('refX',10).attr('refY',5).attr('markerWidth',4).attr('markerHeight',4)
      .attr('orient','auto').append('path').attr('d','M0,0L0,10L10,5Z').attr('fill','#5ec962');
    ...
    // Root
    drawMini(svg,boardsData.root.board,xState,lanes[1]-BH_ROOT/2,BW_ROOT,BH_ROOT,CS_ROOT,'State');
    ...
    actions.forEach((a,i)=>{
      const y=lanes[i];
      svg.append('line').attr('x1',xState+BW_ROOT).attr('y1',lanes[1])
        .attr('x2',xAction+BW_ACT/2).attr('y2',y)
        .attr('stroke','#000').attr('stroke-width',2)
        //.attr('stroke-dasharray','4,2').attr('marker-end','url(#mBlack)');
      drawMini(svg,a.board,xAction-BW_ACT/2 + 70,y-BH_ACT-12,BW_ACT,BH_ACT,CS_ACT,'Action');
      svg.append('line').attr('x1',xAction+BW_ACT/2).attr('y1',y)
        .attr('x2',xResult-20).attr('y2',y)
        .attr('stroke','#000').attr('stroke-width',2)
        .attr('marker-end','url(#mGreen)');
      svg.append('text')
        .attr('x',(xAction+BW_ACT/2 + xResult-20)/2).attr('y',y-6)
        .attr('text-anchor','middle').attr('fill','#000').attr('font-size',12)
        .text('Flow: '+a.flow.toFixed(1));
      drawMini(svg,boardsData.results[i].board,xResult,y-BH_ROOT/2,BW_ROOT,BH_ROOT,CS_ROOT)
        //`Result (${boardsData.results[i].reward})`);
    });
